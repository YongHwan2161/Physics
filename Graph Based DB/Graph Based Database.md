...
- 문제는 새로운 데이터를 저장할 때, 기존의 토큰들을 연결하여 데이터를 표현할 수 있는데, 데이터가 저장되었음을 나타내는 node를 새로 하나 추가할 것인지이다. 기존의 방식에서는 새로운 data가 추가될 때마다 node를 하나씩 새로 할당하였다. 그리고 새로 할당받은 node의 ch 0에서부터 token cycle이 axis 2로 연결되고, 해당 node의 데이터를 불러올 때에는  ch 0의 axis 2를 순회하면 되는 방식이었다. 